  ...
  const handleUpdatePhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        setIsSaving(true);
        const { error } = await supabase.from('perfis').update({ foto_url: reader.result as string }).eq('id', profileData.id);
        if (!error) onRefresh();
        setIsSaving(false);
      };
      ...
      <div className="bg-white p-12 rounded-apple-2xl shadow-sm border border-gray-50 flex flex-col md:flex-row gap-10 items-center md:items-start relative overflow-hidden">
        ...
        <div className="flex-1 text-center md:text-left space-y-4 z-10">
          <div className="flex flex-col md:flex-row md:items-center gap-3">
            <h2 className="text-3xl font-black text-ejn-teal tracking-tight">{profileData.nome}</h2>
          ...
          <div className="bg-white p-10 rounded-apple-2xl shadow-sm border border-gray-50">
            ...
            <div className="flex gap-4">
              {profileData.linkedin && (
                <a href={profileData.linkedin} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-blue-50 text-blue-700 px-4 py-2 rounded-full text-sm font-bold">
                  <Linkedin className="w-4 h-4" /> LinkedIn
                </a>
              )}
              {profileData.instagram && (
                <a href={`https://instagram.com/${profileData.instagram.replace('@','')}`} target="_blank" rel="noreferrer" className="flex items-center gap-2 bg-pink-50 text-pink-700 px-4 py-2 rounded-full text-sm font-bold">
                  <Instagram className="w-4 h-4" /> Instagram
                </a>
              )}
              {!profileData.linkedin && !profileData.instagram && (
                <p className="text-sm text-apple-text-secondary italic">Redes sociais não vinculadas.</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-8">
          <div className="bg-ejn-teal p-8 rounded-apple-2xl text-white shadow-xl relative overflow-hidden group">
            ...
            <p className="text-teal-100/60 text-[10px] font-bold uppercase tracking-widest mb-1">
              {isGestor ? 'Captação Gerenciada' : 'Total Investido'}
            </p>
            ...
            <div className="flex items-center gap-2 text-ejn-gold">
              ...
              <span className="text-xs font-bold uppercase tracking-widest">Sincronizado via Supabase</span>
            </div>
          </div>
          
          <button onClick={() => window.location.reload()} className="w-full bg-apple-gray text-ejn-teal py-4 rounded-apple-xl font-bold text-sm hover:bg-white transition-all border border-gray-200">
